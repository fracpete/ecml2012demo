...
\paragraph{N-to-M semantics.} While a tree representation cannot represent N-to-M relationships, ADAMS solves this shortcoming through \emph{variables, global key-value pairs, and global actors}. Any string token can be assigned to a variable using a \texttt{SetVariable} actor (e.g., \texttt{@\{timestamp\}} in Fig.\ref{flow}), and used as an actor parameter or reintroduced elsewhere (within the scope) as a token by the \texttt{Variable} actor. Similarly, any object token can be stored as a key-value pair by the \texttt{SetStorageValue} actor and reintroduced by the \texttt{StorageValue} actor. Even actors and their sub-flows can be made global: for instance, all tokens sent to a \texttt{GlobalSink} actor are passed to the referenced global actor, as shown in Fig. \ref{flow}. One can limit the scope to a sub-flow by using the \texttt{LocalScope} control actor.
