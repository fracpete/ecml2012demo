...
%
\institute{Leiden University, Leiden, NL, \email{joaquin@liacs.nl}
\and
...
\begin{abstract}
We demonstrate the practical use of a novel, flexible workflow engine, the Advanced Data mining And Machine learning System (ADAMS), aimed at quickly building and maintaining complex knowledge workflows. Compared to most current workflow engines, ADAMS does not require the user to manually connect inputs to outputs on a large canvas, but instead arranges workflows automatically in a very compact tree structure, using \emph{control operators} to direct, split or converge data flows. It contains an extensive library of operators, and a convenient plug-in architecture to easily add new ones.
...
Though this is a very intuitive approach that greatly appeals to many end users, it is also a very time consuming one. When inserting additional operators, the user has to move and rearrange the entire workflow in order to keep the design tidy. If an operator is replaced, all connections have to be redrawn. Moreover, many scientific data analysis workflows are very complex and can involve hundreds of independent steps \cite{bowers}. On a canvas, this leads to very large and complex graphs with many interconnections. This means that oversight is easily lost, even with useful features such as zooming, hierarchical workflows or meta-operators with internal workflows.

In this paper, we present ADAMS (Advanced Data mining And Machine learning System), a novel workflow engine specifically designed for quick prototyping of complex scientific workflows, taking away the need to manually lay out and connect operators on a canvas. It presents the workflow in a compact tree structure in which operators can quickly be dragged in or pulled out, uses a simplified input/output model that allows it to automatically arrange the workflow, and includes \emph{control operators} to create and direct sub-flows. This allows the user to maintain oversight in large complex workflows, and focus entirely on quick workflow prototyping.
...
Figure \ref{flow} illustrates an ADAMS workflow. It reads a file with earthquake data and plots it.\footnote{Simpler flows exist. This workflow is designed to explain ADAMS' control actors.} As in canvas-based systems, operators (called `actors' in ADAMS) are dragged from a library into the workflow. However, in ADAMS, they will automatically `snap' into the tree structure depending on where they are dropped. Actors are represented by small nodes with a (custom) name and a list of parameter settings (options). They are
color-coded based on whether they are a \emph{source} (only output, e.g. \texttt{SingleFileSupplier}~\icon{source}), \emph{sink} (only input, e.g. \texttt{SequencePlotter}~\icon{sink}), \emph{transformer} (input and output, e.g. \texttt{Convert}~\icon{transformer}) or \emph{standalone} (no input or output, e.g. \texttt{GlobalActors}~\icon{standalone}). Branches can be collapsed, and clicking actors opens a settings dialog.
...
\label{examples}
ADAMS is being used in two practical applications involving large, complex workflows. First, Gas Chromatography Mass Spectrometry (GC-MS) is a technique used to detect high concentrations of compounds of interest, but the raw data produced by GC-MS devices is generally not amenable to processing with machine learning systems. Using ADAMS, effective data flows were designed to entirely automate this process \cite{holmes2010}. Second, in the InfraWatch project \cite{infrawatch}, a heterogeneous sensor network of over 150 sensors is monitoring the dynamic behavior and structural health of a highway bridge. The token-based design of ADAMS proved ideal for online processing of sensor data, and its quick workflow prototyping facilitates experimentation with novel time series analysis techniques.

\section{Conclusions}
\label{conclusion}
Most scientific workflow engines use a canvas on which operators are manually arranged and connected. While this is certainly very intuitive and appealing for many end users, it is not ideal for handling very large, complex workflows. ADAMS was developed to offer an alternative for researchers and practitioners dealing with such large workflows. Using a range of \emph{control operators}, it creates a very compact workflow representation in which operators are automatically arranged, thus appreciatively speeding up workflow design and maintenance. Examples and extensive documentation can be found on the ADAMS website: \url{http://www.cs.waikato.ac.nz/~fracpete/adams-demo/}
...
\bibitem{infrawatch}
Vespier, U., Knobbe, A., Vanschoren, J., Miao, S., Koopman, A., Obladen, B., Bosma, C.:Traffic Events Modeling for Structural Health Monitoring. Proceedings of IDA 2011, pp. 376-387 (2011).
